...
_REPORT_DESCRIPTIONS = {
    ...
    "Sap_Purchase": "Raw SAP purchase requisition data: requisitioner, PR processing status "
                    "(PR_Processing_Status), PR creation indicator (system/MRP-generated vs "
                    "manually created), purchasing group, deletion indicator, release info, "
                    "org/plant/department. Use for 'who raised PR X', 'requisitioner for PR', "
                    "'how many PR raised by <person>', 'processing status of PR X', 'processing "
                    "status wise PR count', 'system generated PR', 'PR raised by purchasing group "
                    "X'. Do NOT use PR_release for 'processing status' questions -- PR_release's "
                    "release_status/PR_Released/PR_Rejected track the approval WORKFLOW state, a "
                    "different concept from Sap_Purchase's PR_Processing_Status.",
...
def _build_system_prompt() -> str:
    ...
    parts.append(
        ...
        "group_by_column='department'; 'processing status wise total PR count' -> "
        "group_by_column='pr_processing_status'; 'purchasing group wise PR count' -> "
        "group_by_column='purchasing_group'). group_by_column uses the same semantic keys as "
        "filters. For aggregate questions with a grouping dimension "
        ...
        "applicable: po_number, pr_number, plant, company_code, vendor_name, name_of_supplier, "
        "material_desc, material_code, department, release_status, gate_entry_status, "
        "rejected_at_level, requisitioner, created_by, purchasing_group, creation_indicator, "
        "pr_processing_status, pr_deletion_indicator, service_entry_sheet, current_level, "
        "hod_name, cfo_name, md_name, process_owner_name, vc_chairman_name. Only "
        "include keys the question actually specifies. If the user gives multiple values for one "
        "filter using commas, 'and', or 'or' (e.g. 'sales and admin department', 'PO 5400010477 "
        "and 5400010478'), set that filter value to an array of strings, not one comma-joined "
        "string.\n"
        "- For PR_release questions, 'rejected'/'not approved'/'not accepted' and "
        "'approved'/'accepted'/'not rejected' are release_status values -- do not invent a "
        "separate filter key for them, just set release_status to the word the user used.\n"
        ...
        "- A bare number in a Sap_Purchase question is AMBIGUOUS -- use the surrounding words to "
        "pick the right key, never default to pr_number just because it's numeric:\n"
        "  * 'PR <number>' / 'PR number <number>' / 'purchase requisition <number>' -> "
        "pr_number (the PR document number itself, e.g. 1000016037).\n"
        "  * 'created by <number>' / 'PR created by user <number>' -> created_by (the numeric "
        "SAP user ID that created the PR, e.g. 10001834 -- NOT the PR number, NOT requisitioner).\n"
        "  * 'raised by <name>' / 'created by <name>' / 'requisitioner <name>' (a person's name, "
        "not a number) -> requisitioner.\n"
        "  Example: 'Show processing status of PR 1000016037' -> report=Sap_Purchase, "
        "filters={'pr_number': '1000016037'}. 'PR created by 10001834' -> filters={'created_by': "
        "'10001834'}. Both can look like a bare number in the sentence -- 'PR' immediately before "
        "the number means pr_number; 'created by'/'raised by' immediately before a number means "
        "created_by.\n"
        "- 'purchasing group' / 'purchasing grp' / 'purch group' names a group code (e.g. 'IT "
        "assets', 'C43') -> filters={'purchasing_group': '<the code/text>'}. Do not treat the "
        "group name as a requisitioner or material_desc filter.\n"
        "- 'system generated PR' / 'system created PR' / 'auto-created PR' / 'PR via MRP' -> "
        "filters={'creation_indicator': 'R'}. 'Manually created PR' / 'manual PR' -> "
        "filters={'creation_indicator': 'V'}.\n"
        "- department and requisitioner (or any other listed keys) can and should be combined in "
        "the same filters dict when the question names more than one -- e.g. 'PR created by Lipi "
        "for admin department' -> filters={'requisitioner': 'Lipi', 'department': 'admin'}.\n"
        "- For 'who raised/created PR X' style questions, use operation='list_distinct' (not "
        "'list') -- SAP purchase data is item-level, so a plain 'list' can repeat the same "
        "requisitioner once per line item of the PR.\n"
...
_KPI_PHRASE_RULES = (
    ...
    (
        re.compile(
            r"\bpr(?:s)?\b.*\b(?:release(?:d)?|unrelease(?:d)?|release\s+status)\b"
            r"|\b(?:release(?:d)?|unrelease(?:d)?|release\s+status)\b.*\bpr(?:s)?\b",
            re.IGNORECASE,
        ),
        "pr-release-status",
    ),
...
_GROUP_BY_PHRASE_RULES = (
    ...
    (
        re.compile(
            r"\b(?:processing\s+status\s*[-\s]?wise|by\s+processing\s+status|"
            r"per\s+processing\s+status|processing\s+status\s+breakdown)\b",
            re.IGNORECASE,
        ),
    ...
    (
        re.compile(
            r"\b(?:purchasing\s+gr(?:ou)?p\s*[-\s]?wise|by\s+purchasing\s+gr(?:ou)?p|"
            r"per\s+purchasing\s+gr(?:ou)?p|purchasing\s+gr(?:ou)?p\s+breakdown)\b",
            re.IGNORECASE,
        ),
        "purchasing_group",
    ),
    (
        re.compile(
            r"\b(?:requisitioner\s*[-\s]?wise|by\s+requisitioner|per\s+requisitioner|"
            r"requisitioner\s+breakdown)\b",
            re.IGNORECASE,
        ),
        "requisitioner",
    ),
...
def _sanitize_date_phrase(date_phrase: Optional[str]) -> Optional[str]:
    ...
    if not date_phrase or not date_phrase.strip():
        return None
    # LLMs sometimes serialize JSON null as the literal string "null".
    # Treat it as missing, not as a date phrase.
    if date_phrase.strip().lower() in {"null", "none", "n/a", "na"}:
        return None
...
def _is_null_literal(value) -> bool:
    """LLMs sometimes serialize JSON null as the literal string 'null' (etc). True if
    `value` is one of those string stand-ins for missing, rather than a real value."""
    return isinstance(value, str) and value.strip().lower() in _NULL_LITERALS
...
def _apply_deterministic_overrides(question: str, intent: ExtractedIntent) -> ExtractedIntent:
    ...
    if _is_null_literal(intent.distinct_key):
        updates["distinct_key"] = None
    ...
    # The model can also set kpi_id itself even when none of the deterministic
    # phrase patterns above fired (e.g. a typo like "prr" that \bpr\b can't match,
    # or wording the fixed regex list doesn't cover). Treat that as equally
    # authoritative so the normalization below isn't skipped just because the
    # spotter stayed silent -- otherwise an LLM-set kpi_id sails through with
    # whatever operation/group_by_column it came with, unchecked.
    effective_kpi = matched_kpi or (intent.kpi_id if intent.kpi_id in sqb.KPI_REGISTRY else None)

    if effective_kpi:
        report, _fn, _desc = sqb.KPI_REGISTRY[effective_kpi]
        operation = intent.operation
        distinct_key = updates.get("distinct_key", intent.distinct_key)

        # A KPI's SQL is only capable of grouping when built as count/count_distinct
        # (sql_builder._kpi_count_sql) -- 'aggregate', 'list', 'group_by_count' etc.
        # all fall through to the KPI's fixed, ungrouped detail query and the
        # group_by_column is silently never applied. And even for count/count_distinct,
        # not every KPI has every dimension. Validate against what the KPI actually
        # supports; drop the group_by rather than letting an incompatible column
        # reach the SQL builder as a hard error.
        group_by_column = current_group_by
        if group_by_column and not sqb.kpi_supports_group_by(effective_kpi, group_by_column):
            logger.warning(
                "group_by_column %r dropped -- not a supported dimension for KPI %r -- "
                "question=%r", group_by_column, effective_kpi, question,
            )
            group_by_column = None

        if effective_kpi in sqb.DURATION_DETAIL_KPIS:
            # po-approval-cycle-time / po-approval-delay-level-wise already return
            # the approval-duration metric as detail rows from their own builder fn
            # (sql_builder.kpi_po_approval_cycle_time / kpi_po_approval_delay_level_wise).
            # "how many days were taken to approve" and "level wise ... days" describe
            # the duration metric itself, not a document count -- routing them through
            # count_distinct (as the branches below would, on "how many"/group_by)
            # replaces the duration calculation with COUNT(DISTINCT POR_PO) and reports
            # a PO count instead. Force plain 'list' so the correct detail SQL is used.
            operation = "list"
            group_by_column = None
            distinct_key = None
        elif requested_time_grain:
            operation = "trend"
            distinct_key = distinct_key or _KPI_DEFAULT_DISTINCT_KEYS.get(effective_kpi)
        elif _COUNT_PHRASE_RE.search(question) or group_by_column:
    ...
    # Safety net: a time_grain is only ever meaningful for operation='trend'
    # (sql_builder reads intent.time_grain nowhere else). If the model set one
    # without any trend wording in the question and nothing above established
    # a real trend, it's a hallucinated field -- drop it so it can't silently
    # mislabel a non-trend intent.
    final_operation = updates.get("operation", intent.operation)
    if (
        not requested_time_grain
        and final_operation != "trend"
        and intent.time_grain
        and "time_grain" not in updates
    ):
        updates["time_grain"] = None

    # Sap_Purchase is item-level (see _REPORT_DESCRIPTIONS): a single PR can span
    # multiple SAP line items, and document-level attributes -- processing status,
    # department, requisitioner, etc. -- repeat identically on every one of that
    # PR's rows. sql_builder.build_sql's 'list' branch has no DISTINCT/de-dup step
    # (only 'list_distinct' does), so "processing status of PR X" with operation
    # left at the generic 'list' default comes back as N identical rows instead of
    # one. Upgrade to list_distinct whenever the intent narrows to a single specific
    # PR on this report -- mirrors the existing carve-out already used for "who
    # raised/created PR X" questions, generalized to any single-PR attribute lookup.
    final_operation = updates.get("operation", intent.operation)
    final_kpi_id = updates.get("kpi_id", intent.kpi_id)
    if final_operation == "list" and not final_kpi_id and intent.report == "Sap_Purchase":
        pr_number_filter = (intent.filters or {}).get("pr_number")
        is_single_pr = isinstance(pr_number_filter, str) or (
            isinstance(pr_number_filter, list) and len(pr_number_filter) == 1
        )
        if is_single_pr:
            updates["operation"] = "list_distinct"
