...
class ExtractedIntent(BaseModel):
    ...
    aggregate_function: Optional[str] = Field(
        None,
        description="SUM | AVG | MIN | MAX -- only used when operation == 'aggregate'",
    )
    ...
    group_by_column: Optional[str] = Field(
        None,
        description="Semantic key (e.g. 'department', 'plant', 'vendor_name') to group by",
    )

    time_grain: Optional[str] = Field(
        None,
        description="month | quarter | year -- only used when operation == 'trend'",
    )

    distinct_key: Optional[str] = Field(
        None,
        description="Semantic key to COUNT(DISTINCT ...) on, e.g. 'po_number'",
    )
    ...
    date_phrase: Optional[str] = Field(
        None,
        description="Raw date phrase as the user said it, e.g. 'last month', 'March 2021'. "
                    "Null if no date filter should be applied.",
    )

    limit: Optional[int] = Field(
        None,
        description="Row limit if the user asked for top N",
    )
...
class ChatRequest(BaseModel):
    ...
    execute: bool = Field(
        True,
        description="If false, only return the generated SQL, don't run it",
    )
...
class ChatResponse(BaseModel):
    ...
    # True when the user named no period and the window was defaulted.
    ...
    # Dashboard/graph generated from the exact returned Purchase data.
